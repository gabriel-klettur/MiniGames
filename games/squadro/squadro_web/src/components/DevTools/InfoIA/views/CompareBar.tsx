...
const CompareBar: React.FC<CompareBarProps> = ({ compareSets, onAdd, onRemove, onClear }) => {
  return (
    <div className="compare-bar flex items-center flex-wrap gap-2 my-2">
      <Button size="sm" variant="neutral" onClick={onAdd} title="Agregar — Añade un dataset externo (importado) para comparar en gráficos. Ejemplo: comparar resultados entre commits/configuraciones.">Agregar</Button>
      <Button size="sm" variant="outline" onClick={onClear} disabled={compareSets.length === 0} title="Limpiar — Quita todos los datasets de comparación cargados.">Limpiar</Button>
      <div className="flex items-center flex-wrap gap-2">
        {compareSets.map((s) => (
          <span
            ...
            title={`${s.name} — Dataset de comparación. El color identifica la serie en los gráficos. Usa el ícono para eliminarlo.`}
          >
            <span className="inline-block w-2 h-2 rounded-full bg-white/80" />
            <span className="font-medium drop-shadow-sm">{s.name}</span>
            <Button size="sm" variant="danger" onClick={() => onRemove(s.id)} title="Eliminar — Quita este dataset de la lista de comparación.">
