  ...
  return (
    <div className="mt-2">
      <div className="flex items-center gap-3">
        <span className="inline-flex items-center gap-1 text-xs text-neutral-300" title="Número de jugada en curso" aria-label="Jugada actual">
          <strong className="font-semibold">Jugada</strong>
          <span className="font-mono">{moveIndex}</span>
        </span>
        <div className="relative h-2 flex-1 rounded-md border border-neutral-700 bg-neutral-900 overflow-hidden" title="Progreso de tiempo de la jugada" aria-label="Barra de tiempo de la jugada">
          <div
            className="absolute inset-y-0 left-0 bg-blue-600 transition-[width] duration-100 ease-linear"
            style={{ width: `${Math.round(ratio * 100)}%` }}
          />
        </div>
        <span className="inline-flex items-center gap-1 text-xs text-neutral-300 font-mono" title="Tiempo transcurrido y objetivo">
