...
export function useRecords(_opts: UseRecordsOptions = {}): UseRecords {
  ...
  const exportCSVDetails = useCallback(() => {
    try {
      const header = 'id,startedAt,durationMs,moves,winner,p1Depth,p2Depth,moveIndex,moveElapsedMs,moveAt,depthReached,nodes,nps,score,player,depthUsed,applied\n';
      const lines: string[] = [];
