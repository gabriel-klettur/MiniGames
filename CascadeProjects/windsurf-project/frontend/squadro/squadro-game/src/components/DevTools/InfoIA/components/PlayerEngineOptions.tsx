  ...
  return (
    <div className="engine-opts mt-2 flex flex-col gap-3">
      ...
      <div className={"inline-flex items-center gap-2" + hlWrap('orderingJitterEps')}>
        ...
        <div className={"inline-flex items-center gap-2" + hlWrap('enableKillers')} title="Killer moves — Jugadas que causaron cortes β en el mismo nivel (ply) se prueban antes en ramas hermanas. Ejemplo: si mover P3 en este ply produjo un cutoff en otra rama, aquí también lo intentaremos temprano para forzar nuevas podas.">
        ...
        <div className={"inline-flex items-center gap-2" + hlWrap('enableHistory')} title="History heuristic — Puntúa jugadas por su éxito histórico elevando su prioridad en el orden. Ejemplo: si la jugada 'P2' frecuentemente mejora α, su puntuación de historia crece y se probará antes incluso sin ser táctica.">
        ...
        <div className={"inline-flex items-center gap-2" + hlWrap('preferHashMove')} title="Hash move (desde TT) — Si la TT sugiere una bestMove para la posición, se intenta primero para maximizar cortes β tempranos. Ejemplo: si TT recomienda mover 'P5', se eleva al frente del ordering para explorarla antes.">
        ...
        <div className={"inline-flex items-center gap-2" + hlWrap('enableQuiescence')} title="Quiescence — Extiende hojas tácticas (p. ej., capturas o swings) unos plies adicionales para estabilizar la evaluación y mitigar el 'horizon effect'. Recomendada ON con límite de qPlies moderado (3–5).">
        ...
        <div className={"inline-flex items-center gap-2" + hlWrap('enableTablebase')} title="Tablebase — Atajo O(1) para posiciones conocidas (win/loss/draw) con mejor jugada. Si hay hit, devuelve inmediatamente sin buscar.">
        ...
        <div className={"inline-flex items-center gap-2" + hlWrap('enableDFPN')} title="DF-PN — Activar búsqueda Proof-Number en finales pequeños. Útil para resolver subárboles con pocas piezas activas. Trigger por nº de piezas activas.">
      ...
      <section className="rounded-lg border border-neutral-700 bg-neutral-900/40 p-3">
        ...
        <label className={labelCls + ' flex flex-col gap-1'} title="Quiescence:standPatMargin — Margen restado al stand‑pat para cortes. 0 desactiva.">
          standPatMargin
        ...
        <label className={labelCls + ' flex flex-col gap-1'} title="Quiescence:seeMargin — Umbral tipo SEE para expandir jugadas tácticas.">
          seeMargin
        ...
        <div className={"inline-flex items-center gap-2" + hlWrap('quiescenceExtendOnRetire')} title="Extender quiescence si una jugada retira una pieza (no consume qDepth).">
        ...
        <div className={"inline-flex items-center gap-2" + hlWrap('quiescenceExtendOnJump')} title="Extender quiescence si una jugada produce un salto inmediato.">
      ...
      <section className="rounded-lg border border-neutral-700 bg-neutral-900/40 p-3">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
          <div className={"inline-flex items-center gap-2" + hlWrap('enableLMP')} title="LMP — Poda jugadas muy tardías no tácticas a baja profundidad. Acelera pero no es estrictamente exacto.">
          ...
          <label className={labelCls + ' flex flex-col gap-1'} title="lmpMaxDepth — Aplica LMP cuando depth ≤ este valor.">
          ...
          <label className={labelCls + ' flex flex-col gap-1'} title="lmpBase — Umbral base del índice 'late'. Umbral = lmpBase + 2*depth.">
            <span>lmpBase</span>
          ...
          <div className={"inline-flex items-center gap-2" + hlWrap('enableFutility')} title="Futility — Poda nodos poco prometedores (no‑PV, baja profundidad, no tácticos). Acelera pero no es estrictamente exacto.">
          ...
          <label className={labelCls + ' flex flex-col gap-1'} title="futilityMargin — Margen por ply usado para decidir la poda de futilidad.">
      ...
      <section className="rounded-lg border border-neutral-700 bg-neutral-900/40 p-3">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
          <div className={"inline-flex items-center gap-2" + hlWrap('enableAspiration')} title="Aspiration windows — Empieza con una ventana alrededor del score previo y re‑busca si falla.">
          ...
          <label className={labelCls + ' flex flex-col gap-1'} title="aspDelta — Semiancho inicial de la ventana de aspiración.">
            <span>aspDelta</span>
          ...
          <div className={"inline-flex items-center gap-2" + hlWrap('enableIID')} title="IID — Sonda a depth‑1 para sembrar el orden cuando no hay hash move.">
          ...
          <label className={labelCls + ' flex flex-col gap-1'} title="iidMinDepth — Solo aplicar IID cuando la profundidad actual sea ≥ este valor.">
      ...
      <section className="rounded-lg border border-neutral-700 bg-neutral-900/40 p-3">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
          <div className={"inline-flex items-center gap-2" + hlWrap('enableAdaptiveTime')} title="Tiempo adaptativo — Evita iniciar una iteración si el tiempo restante no alcanza. Solo tiene efecto con límite de tiempo finito.">
          ...
          <label className={labelCls + ' flex flex-col gap-1'} title="timeSlackMs — Margen (ms) reservado para evitar pasarse del presupuesto.">
          ...
          <label className={labelCls + ' flex flex-col gap-1'} title="adaptiveGrowthFactor — Crecimiento base estimado del coste de la próxima iteración.">
          ...
          <label className={labelCls + ' flex flex-col gap-1'} title="adaptiveBFWeight — Peso extra por factor de ramificación en raíz (ajusta el crecimiento).">
            <span>bfWeight</span>
      ...
      {(typeof p.w_race === 'number' || typeof p.w_clash === 'number' || typeof p.w_sprint === 'number' || typeof p.w_block === 'number') && (
        <section className="rounded-lg border border-neutral-700 bg-neutral-900/40 p-3">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
            <label className={labelCls + ' inline-flex items-center gap-2' + hlWrap('w_race')} title="w_race — Peso de carrera: recompensa tener menos turnos restantes. Ejemplo: si tú tienes 6 turnos totales y rival 8, un w_race alto favorece tu posición por ventaja de carrera.">
              w_race
            ...
            <label className={labelCls + ' inline-flex items-center gap-2' + hlWrap('w_clash')} title="w_clash — Choques inminentes: pondera el swing de turnos por saltos previstos en 1–2 plies. Ejemplo: si puedes enviar atrás 1 pieza rival en tu turno y no quedas expuesto al siguiente, el score aumenta.">
              w_clash
              <input type="number" step={0.1} disabled={dis} className={inputCls + hlInput('w_clash')} value={p.w_clash ?? 0.8} onChange={(e) => p.onChangeWClash?.(Number(e.target.value))} />
            </label>
            <label className={labelCls + ' inline-flex items-center gap-2' + hlWrap('w_sprint')} title="w_sprint — Sprint final: incentiva cerrar cuando alguna pieza está a pocos turnos de retirarse. Ejemplo: si una pieza está a ≤ sprint_thr, se suma un bonus proporcional.">
              w_sprint
              <input type="number" step={0.1} disabled={dis} className={inputCls + hlInput('w_sprint')} value={p.w_sprint ?? 0.6} onChange={(e) => p.onChangeWSprint?.(Number(e.target.value))} />
            </label>
            <label className={labelCls + ' inline-flex items-center gap-2' + hlWrap('w_block')} title="w_block — Bloqueo útil vs. exposición: favorece tapar rutas rivales en ≤2 pasos y penaliza quedar al alcance inmediato del rival. Ejemplo: si tras tu mejor avance de prueba el rival te alcanza fácil, se resta puntuación.">
              w_block
              <input type="number" step={0.1} disabled={dis} className={inputCls + hlInput('w_block')} value={p.w_block ?? 0.3} onChange={(e) => p.onChangeWBlock?.(Number(e.target.value))} />
            </label>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mt-2">
            <label className={labelCls + ' inline-flex items-center gap-2' + hlWrap('done_bonus')} title="done_bonus — Bonus por piezas retiradas. Ejemplo: cada pieza en 'retirada' suma este bonus (además del progreso de carrera acumulado).">
              done_bonus
            ...
            <label className={labelCls + ' inline-flex items-center gap-2' + hlWrap('sprint_threshold')} title="sprint_thr — Umbral de turnos que activa el bonus de sprint. Ejemplo: con sprint_thr=2, piezas a 1–2 turnos de retirarse disparan el término de sprint (positivo para ti, negativo si es del rival).">
              sprint_thr
            ...
            <label className={labelCls + ' inline-flex items-center gap-2' + hlWrap('tempo')} title="tempo — Iniciativa: bonus suave cuando es tu turno para preferir líneas que conservan la presión. Ejemplo: en posiciones similares, la que te deja mover de nuevo con ventaja obtiene +tempo.">
              tempo
              <input type="number" step={1} disabled={dis} className={inputCls + hlInput('tempo')} value={p.tempo ?? 5} onChange={(e) => p.onChangeTempo?.(Number(e.target.value))} />
            </label>
          </div>
        </section>
      )}
