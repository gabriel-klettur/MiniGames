...
export function ControlPanel(props: ControlPanelProps) {
  const {
    ...
    onToggleAiFailSoft,
    aiPreferHashMove,
    onToggleAiPreferHashMove,
    aiEnablePVS,
    onToggleAiEnablePVS,
    aiEnableAspiration,
    onToggleAiEnableAspiration,
    aiAspirationDelta,
    onChangeAiAspirationDelta,
    aiEnableKillers,
    onToggleAiEnableKillers,
    aiEnableHistory,
    onToggleAiEnableHistory,
    aiEnableQuiescence,
    onToggleAiEnableQuiescence,
    aiQuiescenceDepth,
    onChangeAiQuiescenceDepth,
  } = props;

  // Presupuesto de tiempo (ms) y progreso mostrado
  const limitMs: number | null = timeMode === 'manual' ? Math.max(0, (timeSeconds || 0) * 1000) : null;
  const shownElapsedMs = busy ? (typeof busyElapsedMs === 'number' ? busyElapsedMs : elapsedMs || 0) : (elapsedMs || 0);
